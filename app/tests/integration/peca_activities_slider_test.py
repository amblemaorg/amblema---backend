# app/tests/peca_activities_slider_test.py
...
class PecaSchoolTest(unittest.TestCase):
    ...
    def test_update_activities_slider(self):

        requestData = {
            "slider": [test_image, test_image]
        }
        res = self.client().put(
            '/pecaprojects/activitiesslider/{}?userId={}'.format(
        ...
        self.assertEqual(True,
                         result['isInApproval'])
        self.assertEqual(2,
                         len(result['approvalHistory'][0]['detail']['slider']))

        res = self.client().get(
            '/pecaprojects/{}'.format(self.pecaProject.id))
        result = json.loads(res.data.decode('utf8').replace("'", '"'))
        self.assertEqual(True,
                         result['school']['activitiesSlider']['isInApproval'])
        self.assertEqual(2,
                         len(result['school']['activitiesSlider']['approvalHistory'][0]['detail']['slider']))
        ...
        self.assertEqual(2,
                         len(request['detail']['slider']))
        self.assertEqual('9', request['type'])
        ...
        result = json.loads(res.data.decode('utf8').replace("'", '"'))
        self.assertEqual(2,
                         len(result['school']['activitiesSlider']['slider']))

        # check school
        res = self.client().get(
            '/users/{}?userType=4'.format(self.school.id))
        resultSchool = json.loads(res.data.decode('utf8').replace("'", '"'))
        self.assertEqual(2, len(resultSchool['activitiesSlider']))

        # delete image
        requestData['slider'] = resultSchool['activitiesSlider']
        ...
        res = self.client().get(
            '/users/{}?userType=4'.format(self.school.id))
        resultSchool = json.loads(res.data.decode('utf8').replace("'", '"'))
        self.assertEqual(1, len(resultSchool['activitiesSlider']))
